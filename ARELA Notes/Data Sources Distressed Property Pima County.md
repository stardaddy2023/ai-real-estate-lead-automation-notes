...
* **The "API" Connection:** The City of Tucson has its own Open Data portal (separate from the County) which is excellent for code violations.  
  ...
  * **API Endpoint:** `https://gis.tucsonaz.gov/arcgis/rest/services/PDSD/pdsdMain_General5/MapServer/94`
  * **Layer ID:** **94** ("Violations")
  * **Data:** Updated daily from Permis+ system. Includes open and closed cases.
...
### **7\. Advanced GIS Layers (Investor Goldmine)**

Beyond the basic "Parcels" layer, Pima County exposes several other layers that are highly valuable for investors. These can be accessed via the same ArcGIS REST API.

*   **Base URL:** `https://gisdata.pima.gov/arcgis1/rest/services/GISOpenData`

#### **A. Zoning & Land Use (Value Add Potential)**
*   **Service:** `OverlayZoningBase/MapServer`
*   **Key Layers:**
    *   **Layer 1:** `Zoning - Pima County` (County Zoning Codes)
    *   **Layer 3:** `Zoning - City of Tucson` (City Zoning Codes)
    *   **Layer 17:** `Overlay Zones` (Historic Districts, etc.)
*   **Investor Use:** Identify properties with rezoning potential or those in restrictive overlay districts (Historic) that might complicate flips.

#### **B. Development Activity (Path of Progress)**
*   **Service:** `OverlayDevelopment/MapServer`
*   **Key Layers:**
    *   **Layer 13:** `Tentative Plat Plans` (New subdivisions coming soon)
    *   **Layer 7:** `Improvement District Plans`
    *   **Layer 9:** `Landscape Plans`
*   **Investor Use:** Spot where new development is happening ("Path of Progress"). Buying land or fixers near approved tentative plats can be a great appreciation play.

#### **C. Floodplain & Hazards (Risk Assessment)**
*   **Service:** `OverlayFloodplainPCRFCD/MapServer`
*   **Key Layers:**
    *   **Layer 2:** `Erosion Hazard Setback`
*   **Investor Use:** Quickly filter out properties that might have uninsurable flood risks or expensive build requirements.

#### **D. School Districts (Rental Demand)**
*   **Service:** `OverlaySchools/MapServer`
*   **Key Layers:**
    *   **Layer 0:** `School Districts`
*   **Investor Use:** Map overlay to target properties specifically in high-demand school districts (e.g., Catalina Foothills, Vail).